...
let listContainer = document.getElementById("list__container"), //gets the container the8 list items will be rendered into
...
function collectInputValue(e) { //this works as the name indicates - collect input value
    ...
    let filtered = collectedJsonData.filter( item => { //this subjects the fetched data to filtering using the search input value as a query
        return item.stack.includes(searchInputValue) //the array method ".includes" returns either true or false. 
